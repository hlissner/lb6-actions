...
function parse(input) {
    if (input.indexOf(" ") === -1) {
        // A history is kept of all the successful conversions done in the past
    ...
    // If there are 2 or 4 words, the user is typing the currency codes. This is
    // where the suggestion magic happens.
    if (_input_len === 2 || _input_len === 4) {

        // We chop off what the user is currently typing so we can use this as
        // the root for suggestions.
        var actionArg = _input.splice(0, _input_len-1).join(" ") + " ";

        // API.suggest does a simple string.indexOf on both the currency's full
        // name (e.g. "Danish Krone") and the code (e.g. "DKK"); this makes it
        // easier to type in colloquial names, e.g. "5 yen", and have JPY
        // suggested.
        //
        // Note: splice mutated _input in the line above, so there should only
        // be one word left in _input.
    ...
    // In between 2 and 4 words the user is typing the delimiter (accepted
    // delimiters: to, in, and -); but this will ensure they get "[amt] [CODE]
    // to " suggested.
